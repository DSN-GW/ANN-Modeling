...
def preprocess_training_data(test_size=0.2, random_state=42):
    print("=" * 60)
    ...
    print("Initializing characteristic feature extractor...")
    char_extractor = FeatureExtractor()
    print("Extracting characteristic features from training data...")
    train_processed = char_extractor.process_dataset(train_df)
    print("Initializing NLP feature extractor...")
    nlp_extractor = NLPFeatureExtractor()
    print("Extracting NLP features from training data...")
    train_final = nlp_extractor.process_dataset(train_processed)
    ...
    # Also preprocess and save test data to avoid reprocessing during prediction
    print("Extracting characteristic features from test data...")
    test_processed = char_extractor.process_dataset(test_df)
    print("Extracting NLP features from test data...")
    test_final = nlp_extractor.process_dataset(test_processed)
...
def preprocess_prediction_data(df, is_test_data=False):
    """
    Preprocess prediction data. If is_test_data=True and preprocessed test data exists,
    load it directly. Otherwise, perform live preprocessing.
    """
    print("=" * 60)
    print("PREPROCESSING PREDICTION DATA")
    print("=" * 60)
    
    # If this is test data, try to load preprocessed version first
    if is_test_data:
    ...
    try:
        ...
        print("Creating fresh extractors...")
        char_extractor = FeatureExtractor()
        nlp_extractor = NLPFeatureExtractor()
        print("Extracting characteristic features...")
        processed_df = char_extractor.process_dataset(df)
        print("Extracting NLP features...")
        final_df = nlp_extractor.process_dataset(processed_df)
        ...
        print(f"Error: {e}")
        print("Preprocessing info not found. Please run training first.")
        raise
